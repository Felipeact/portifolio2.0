...
interface ShapesProps {
  isHover: boolean;
  isPress: boolean;
  mouseX: MotionValue<number>; // Update to MotionValue<number>
  mouseY: MotionValue<number>; // Update to MotionValue<number>
}
...
export function Shapes({
  ...
  const lightRotateX = useSmoothTransform({
    ...
    transformers: [mouseToLightRotation],
  });
  ...
  return (
    <Canvas shadows dpr={[1, 2]} resize={{ scroll: false, offsetSize: true }}>
      <Camera mouseX={mouseX} mouseY={mouseY} isHover={false} isPress={false} />
...
export function Sphere(): JSX.Element {
  return (
...
export function Cone(): JSX.Element {
  return (
    <motion.mesh
      position={[-0.8, 0.4, 0]}
      rotation={[-0.5, 0, -0.3]}
...
function Camera({ mouseX, mouseY, ...props }: ShapesProps): JSX.Element {
  
  const cameraX = useSmoothTransform({
    value: mouseX,
    springOptions: spring,
    transformers: [(x : number ) => x / 350],
  });
  ...
  const cameraRef = useRef<any>(); // Replace with the appropriate ref type
...
const mouseToLightRotation: TransformFunction = (value: number): number => {
  // Transform logic using the single value
  return -value / 140;
};