...
import { useBlogs, useLatestBlogs } from '@/services/apiCalls';
import { MoreButton } from '@/components/MoreButton';
import { format } from 'date-fns';
import { motion, AnimatePresence } from 'framer-motion';

const ALL_TAGS = ['UI', 'C++', 'Blueprint'];
    ...
    <div className="md:px-8">
      ...
      <section className="text-center mt-16">
        ...
        <p className="mt-4 text-gray-700">
          Insights on game development, software engineering, and tech trends.
        </p>
      </section>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        {latestBlogs.map(index => (
          <div className="bg-white shadow-md rounded-lg p-4" key={index.id}>
            <Image
              src="/unreal-engine.jpeg"
            ...
            <div className="flex items-center justify-between ">
              ...
              <div className="flex gap-2 items-center text-sm text-gray-500 mt-[2rem]">
                ...
                <p>{format(new Date(index.createdAt), 'PPP')}</p>
              </div>
            </div>
          </div>
        ))}
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute top-12 bg-white border border-gray-300 rounded-md p-4 shadow-md w-72"
            >
